...
<?php include 'app/adms/Views/_includes/navbar.php';?>
						                ...
						                <table class="table table-bordered table-hover dataTables-example">
						                    ...
						                    <tbody>
				    							<?php foreach ($data as $banner): ?>
				    								...
				    								<?php
				    									$image = ($banner['banner']) ? 'app/sts/assets/img/main_banners/' . $banner['banner'] : '';

				    									$status_color = ($banner['status'] == "Ativo") ? "primary" : "danger";
								                    ...
								                    <tr class="gradeX">
								                        ...
								                        <td>
	                                                        ...
	                                                        <input type="number" min="1" value="<?=$banner['orderby']?>" name="orderby" style="display:none; width:80%;" class="form-control">
								                        ...
								                        <td><span class="badge badge-<?=$status_color?> p-xs text-uppercase status_color"><?=$banner['status']?></span></td>
								                        <td>
								                        	<input type="hidden" name="banner_id" value="<?= $banner['id_banner'] ?>">
								                        	<a href="javascript:;" class="btn btn-<?= ($banner['status'] == "Ativo") ? 'warning' : 'primary' ?> btn-m toggle_status" title="<?= ($banner['status'] == "Ativo") ? 'Desativar' : 'Ativar' ?>"><i class="fa fa-check"></i></a>
															<?php if(isset($this->sessionPermi['u_edit'])): ?>
																<a href="<?= $this->pageEdit . '/' . $banner['id_banner']?>" class="btn btn-warning btn-m" title="Editar"><i class="fa fa-pencil"></i></a>
															<?php endif ?>
															<?php if(isset($this->sessionPermi['u_delete'])): ?>
																<a href="<?= $this->pageDelete . '/' . $banner['id_banner']?>" class="btn btn-danger btn-m" onclick="return confirm('Deseja excluir esse registro?')" title="Excluir"><i class="fa fa-trash-o"></i></a>
															<?php endif ?>
								                        </td>
								                    </tr>
												<?php endforeach?>
    ...
    <script src="<?= URLADM ?>app/adms/assets/js/pages/banners/banners-actions.js"></script>

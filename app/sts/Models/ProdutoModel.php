...
class ProdutoModel
{
    ...
    {
        return $this->query['fullRead']->query("SELECT pr.*,ct.name as name_category
    ...
    public function get_product_pictures($id)
    {
        ...
        return $this->query['fullRead']->query("SELECT * FROM pr_products_pictures WHERE id_product = :id_product ORDER BY orderby", [], "id_product={$id}", ['s']);
    ...
    public function get_related_products($category, $id_product)
    {
        ...
        return $this->query['fullRead']->query("SELECT pr.*,ct.name FROM pr_products as pr inner JOIN pr_categories AS ct
